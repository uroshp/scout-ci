...
| `provenance` | object or absent | optional | **Propagation only, by value (2026-09-26).** `{source_url, source_tier, as_of, fact_id, stamped_on}`: the source the judge confirmed this op against, copied onto the claim at apply time. The renderer prefers it over the `derived_from` hop, so a citation can no longer drift when the parent is later revised or re-grounded (the two stale-citation leaks of 2026-09-23 / 09-26). `derived_from` remains the lineage/dependency edge. Not an own-source anchor: it does not exempt or require `evidence_excerpt`/`grounding`. See §2.3. |
...
- **`provenance` (by value).** Inheriting through `derived_from` is resolved at render time, and the parent is mutable: a revise re-anchors it to a newer fact and drops its own source; a re-grounded anchor changes its URL. Both silently changed what dependents cited. Since 2026-09-26 `apply_ops` copies the judge-seen source onto the claim as `provenance`, and `review.apply` runs a deterministic pre-write gate (`provenance_issues`): no active claim may lose its citation, and every applied op must cite the source in the run's logged fact. A failure writes nothing and is reported loudly.
