...
**Materiality-first cure routing (2026-07-31).** The judge's reject logic is no longer a binary "prose-fixable?"; it answers materiality FIRST. On every reject it emits `material` (would the underlying POINT move a deal?) and, if so, `cure` (prose | root | none). `material:false` → drop (immaterial, correct and common). `material:true, cure:"prose"` → the routing is right, rewrite the wording (the pre-existing path). `material:true, cure:"root"` → the deal-moving point is sound but the approach/pivot/mechanism is wrong (the 7/31 containment case: a rebuttal that invented "Azure Foundry governance fixes a model-behavior breach"); the rewrite PRESERVES the point and re-approaches the root — not a phrase-patch. `material:true, cure:"none"` → material but no grounded correct expression exists; do not invent, route to the owner. The judge's `reason` must carry the FULL diagnosis up front (for root: what the point is + the correct grounded approach), fixing the incident's second failure where the honest framing only appeared on the exhausting rejection. `rewritable` is now derived (`material and cure∈{prose,root}`) — the loop gates on `material`. The rewrite budget rose to `PROPAGATE_MAX_REWRITES=3` cure rounds. A material point that can't be cured (exhausted, or `cure:"none"`) is flagged `material_uncured` and fires a SEPARATE URGENT email (`notify.send_urgent_material`, real SMTP) carrying the judge's honest diagnosis — belt-and-suspenders with the proposals email's AUTHORING-FAILED section, so a deal-moving point is never silently dropped. Schema evolution is fully additive (`material`/`cure`/`material_uncured`; decision-log v6); `judge_verdict` semantics unchanged, so the authorship eval corpus (`adjudicate.py`, which never reads these fields) is untouched.

**Supersede-retire sweep (2026-07-25, the no-stale-state philosophy).** A claim either moves deals or it doesn't; there is no third "stale" state. When a routed fact establishes that a named identifier is replaced (a new flagship, a renamed product, a superseded price list), the router names the superseded identifier strings per op (`superseded_terms`, grounded: only strings literally present in the fact or the revised claim survive `verified_superseded_terms`). Code then sweeps every ACTIVE routable claim still citing a verified term (`supersede_candidates` — excluding claims this run already routes, the trigger facts, and claims updated on/after the supersession date, whose mention is deliberate compressed history) into synthesized `supersede_retire` ops that join the ordinary floor + judge batch. The judge decides each with the deal-moving lens — true-but-inert is a correct reason to retire; a claim whose point survives the version change is rejected and stays. Confirmed retires flow through the existing lineage-keeping apply (`retired_reason: "superseded: …"`). For `SUPERSEDE_HUNT_DAYS` (14) after such a retire, triage carries a replacement-hunt block (cutoff-exempt) actively searching for the new identifier's value on the same subject, so the fresh claim arrives via the normal grounded path. Review/live only; a real supersession on a mature card yields ~10 judge candidates (~$0.2–0.4 of judge time), and the sweep is otherwise free.

**Length-cure loop (2026-07-25, "cure, don't hold").** The judge certifies substance; code certifies length. After the rewrite loop, a deterministic length floor (`_demote_overcap_confirms`) demotes any judge-confirmed add/revise whose body exceeds the 170-word render cap (`schema.word_cap_errors` — the same count the render gate enforces) into a rewritable rejection carrying the exact cap violation, and a dedicated cure round (`PROPAGATE_MAX_LENGTH_CURES`, its own budget so a content rewrite can't starve it) condenses it facts-only and blind-re-judges it. An uncured op is RESTORED to its confirmed state — a confirmed material claim never degrades to an exhausted reject — and falls to the render gate, whose repair now also condenses length-only violations (it previously only fixed missing blocks, the root cause of the 7/18 and 7/25 holds). **Re-judge everywhere:** any condense that alters confirmed prose outside the blind-re-judge path (gate / approve / apply backstops) must pass a fidelity judge (`reformat.verify_condense`, Opus with the standard fallback, fail-closed to HOLD) comparing condensed against original — every number, date, name and still-true fact must survive, nothing invented. The reformatter also carries `WRITING_STYLE` (a live condense minted an em dash without it). Cost keys: `length_cure_author/judge`, `reformat`, `gate_judge`; decision-log v5 adds `length_demoted/length_cured/length_cure_attempts/condensed_at_gate/gate_rejudge`.
