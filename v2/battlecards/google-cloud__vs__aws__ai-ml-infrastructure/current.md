...
- On August 26, 2026, AWS signed a definitive agreement to acquire DuckLabs, the Amsterdam company behind DuckDB, the open source in-process analytical database that runs SQL directly against files on local disk or Amazon S3. DuckDB itself stays open source under the MIT license and the independent DuckDB Foundation, and co-founders Hannes Muhleisen and Mark Raasveldt keep leading its technical direction. AWS is buying the engineering team and plans to fold DuckDB's speed on S3 data into its analytics stack (Redshift, Athena, EMR, Glue and SageMaker Lakehouse). The deal is expected to close in early September 2026. ([aboutamazon.com](https://www.aboutamazon.com/news/company-news/aws-ducklabs))
...
**Google ships frontier model gains faster than buyers can finish evaluating the last one.**

On September 2, 2026, Google released Gemini 3.8 Flash, its third Flash-tier model in six weeks after 3.7 Flash landed just three weeks earlier. 3.8 Flash holds the exact same price as 3.7 Flash, \$0.75 per million input tokens and \$3.75 per million output tokens through the end of 2026, while its DeepSWE v1.1 long-horizon coding score climbs to 73.7% from 65.3%. It went live at once across the Gemini app, Gemini Enterprise, Google AI Studio, the Gemini API, Android Studio, Google Antigravity and AI Mode in Google Search, not a staged rollout.

**Soundbite:** Google has shipped three Flash upgrades in six weeks at the same price, each one better at coding than the last, so your model keeps improving without your bill moving. ([blog.google](https://blog.google/innovation-and-ai/models-and-research/gemini-models/3-8-flash-and-3-8-flash-cyber/))
